...
fit_models <- function(problem, N = 1, seed = 123){
  
  cat(paste0("Evaluating: ", problem, "\n"))
  
  set.seed(seed)
  
  if(paste0(problem, ".csv") %in% list.files("classification-models/results/")){ # Prevent unnecessary re-runs
    return(NA)
  } else{
    ...
    write.csv(results, paste0("classification-models/results/", problem, ".csv"), row.names = FALSE)
  }
}
